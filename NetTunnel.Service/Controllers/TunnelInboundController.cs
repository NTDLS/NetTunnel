    ...
    public class TunnelInboundController : ControllerBase
    {
        ...
        public NtActionResponseEndpointStatistics EndpointStatistics(Guid sessionId)
        {
            try
            {
                ...
                {
                    Statistics = Singletons.Core.InboundTunnels.GetEndpointStatistics(),
                    Success = true
                };
            }
            catch (Exception ex)
        ...
        public async Task<NtActionResponse> AddEndpointOutboundPair(Guid sessionId, Guid tunnelId, [FromBody] string value)
        {
            try
            {
                ...
                var result = await Singletons.Core.OutboundTunnels.DispatchAddEndpointInbound<NtFramePayloadBoolean>(tunnelId, endpoints.Inbound);
